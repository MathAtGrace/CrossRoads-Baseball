...
In order to run the Elo model, we needed to scrape the schedule of each team from dakstats.
...
We are using Elo values to predict the outcome of games. The Elo values are based on who won along with the score differential and are calculated using the Elo package in r. We begin with each teams' Elo value at 1500.
